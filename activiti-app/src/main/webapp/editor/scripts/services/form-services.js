    ...
    function ($http, $q, $rootScope, $translate) {
        ...
        this.saveForm = function (data, name, formKey, description, saveCallback, errorCallback) {
            ...
            html2canvas(jQuery('#canvasSection'), {
                onrendered: function (canvas) {
                    ...
                    $http({
                        method: 'PUT',
                        url: ACTIVITI.CONFIG.contextRoot + '/app/rest/form-models/' + $rootScope.currentForm.id,
                        data: data
        ...
        var _updateFormCache = function (stepId, formId) {
            if (stepId && formId) {
                $http({
                    method: 'GET',
                    url: ACTIVITI.CONFIG.contextRoot + '/app/rest/form-models/' + formId
        ...
        var _getFormDefinitions = function (formIds, callback) {
            ...
            if (formIds.length > 0) {
                ...
                $http({
                    method: 'GET',
                    url: ACTIVITI.CONFIG.contextRoot + '/app/rest/form-models/values?' + formIdParams
